...
describe('scaffold', () => {
  ...
  it('passes --pinia only when the store answer is yes', () => {
    expect(vue.scaffold('demo-app', { ...DEFAULT_ANSWERS, store: true }).args).toContain('--pinia');
    expect(vue.scaffold('demo-app', DEFAULT_ANSWERS).args).not.toContain('--pinia');
  });

  it('appends --vitest unless testing is declined', () => {
    const withVitest = vue.scaffold('demo-app', { ...DEFAULT_ANSWERS, testing: 'vitest' });
    const withoutVitest = vue.scaffold('demo-app', { ...DEFAULT_ANSWERS, testing: 'none' });
