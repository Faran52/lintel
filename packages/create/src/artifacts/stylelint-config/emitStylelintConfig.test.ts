...
describe('emitStylelintConfig', () => {
  ...
  it('parses the SFC style block on the two targets that have one', () => {
    const vue = emitStylelintConfig({ ...DEFAULT_ANSWERS, target: 'vue' });
    const svelte = emitStylelintConfig({ ...DEFAULT_ANSWERS, target: 'svelte' });
  ...
  it('hands no SFC syntax to a target with no single-file component', () => {
    expect(emitStylelintConfig({ ...DEFAULT_ANSWERS, target: 'react' })).not.toContain('postcss-html');
    expect(emitStylelintConfig({ ...DEFAULT_ANSWERS, target: 'webextension' }))
