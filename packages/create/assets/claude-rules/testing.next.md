...
- **An async Server Component is not renderable by RTL.** Test what it computes by extracting that
  into a plain function in `lib/` and testing the function; test the markup through the Client
  Component it renders. Do not `await` a component into `render`.
- `next/navigation` (`useRouter`, `usePathname`, `useSearchParams`) has no implementation outside a
  Next request. Mock it globally in setup, never per test.
- Anything in `lib/server/` is `server-only` and will refuse to import into a happy-dom test. Test
  it in a `node`-environment test file (`// @vitest-environment node`).
